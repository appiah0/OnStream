...
import {
  ...
  searchAll, SERVERS, DOWNLOAD, SUBTITLE_LANGS, getExternalDlLinks,
...
function DownloadPanel({ item, isTV, season, episode }) {
  const type  = isTV ? 'tv' : 'movie';
  const links = getExternalDlLinks(type, item.id, season, episode);
  ...
  return (
    <div style={{padding:'14px',flexShrink:0}}>
      <div style={{color:'rgba(255,255,255,.5)',fontSize:12,marginBottom:4,lineHeight:1.5}}>
        Copy a stream link and paste it in <b style={{color:'#fff'}}>Vidmate</b> or <b style={{color:'#fff'}}>IDM</b> to download.
      </div>
      <div style={{color:'rgba(255,255,255,.3)',fontSize:11,marginBottom:14}}>
        In Vidmate: tap + → paste link → it will detect the video stream.
      </div>
      <div style={{display:'flex',flexDirection:'column',gap:10}}>
        {links.map((src,i)=>(
          <div key={i} style={{background:'rgba(255,255,255,.04)',border:'1px solid rgba(255,255,255,.08)',borderRadius:10,padding:'12px 12px'}}>
            <div style={{fontWeight:700,fontSize:13,color:'#fff',marginBottom:3}}>{src.name}</div>
            <div style={{color:'rgba(255,255,255,.35)',fontSize:10,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',marginBottom:9}}>{src.url}</div>
            <div style={{display:'flex',gap:7}}>
              <button className="btn" onClick={()=>copyLink(src.url, i)}
                style={{flex:1,background:copied===i?'rgba(74,222,128,.15)':'rgba(255,255,255,.08)',border:`1px solid ${copied===i?'rgba(74,222,128,.4)':'rgba(255,255,255,.1)'}`,color:copied===i?'#4ade80':'#fff',borderRadius:7,padding:'7px 0',fontSize:12,fontWeight:700,display:'flex',alignItems:'center',justifyContent:'center',gap:5}}>
                <Ic.Copy/>{copied===i?'Copied!':'Copy Link'}
              </button>
              <button className="btn" onClick={()=>window.open(src.url,'_blank')}
                style={{flex:1,background:'#e50914',border:'none',color:'#fff',borderRadius:7,padding:'7px 0',fontSize:12,fontWeight:700,display:'flex',alignItems:'center',justifyContent:'center',gap:5}}>
                <Ic.Share/> Open
              </button>
            </div>
          </div>
...
function Player({ item, onClose, onToggleFav, isFav, settings, onWatched }) {
  ...
  const goFS = () => {
    const el = ifrRef.current || wrapRef.current;
    if (!el) return;
    ...
    if (fn) { fn.call(el); }
    else if (ifrRef.current?.src) { window.open(ifrRef.current.src, '_blank'); }
  };
...
function DetailPage({ item, onPlay, onBack, onToggleFav, isFav, settings, onWatched }) {
  ...
  // Build stream page URLs — these are what Vidmate/IDM use to extract streams
  const getStreamUrl = (quality, epNum = null, epSeason = season) => {
    if (isTV && epNum) {
      return `https://vidsrc.xyz/embed/tv?tmdb=${item.id}&season=${epSeason}&episode=${epNum}`;
    }
    return `https://vidsrc.xyz/embed/movie?tmdb=${item.id}`;
  };

  // Copy URL to clipboard then open in external app
  ...
  // Per-episode download state machine: null → 'links'
  const epDlKey  = (ep) => `ep_${ep}`;
  const [epDlStep,  setEpDlStep]  = useState({});
  const [epDlQual,  setEpDlQual]  = useState({});

  const startEpDl   = (epNum) => setEpDlStep(p => ({...p, [epDlKey(epNum)]: 'links'}));
  const pickEpQ     = (epNum, q) => { setEpDlQual(p => ({...p, [epDlKey(epNum)]: q})); };
  const cancelEpDl  = (epNum) => setEpDlStep(p => ({...p, [epDlKey(epNum)]: null}));

  // Movie download state
  const [movDlStep, setMovDlStep] = useState(null);
  ...
  return (
    <div style={{position:'fixed',inset:0,zIndex:2000,background:'#0a0a0f',overflowY:'auto',overflowX:'hidden'}}>
      ...
      <div style={{padding:'14px 16px',display:'flex',gap:10}}>
        ...
        {!isTV && (
          <button className="btn" onClick={()=>setMovDlStep('links')}
      ...
      {!isTV && movDlStep === 'links' && (
        <div style={{margin:'0 16px 14px',padding:'14px',background:'rgba(255,255,255,.04)',border:'1px solid rgba(255,255,255,.09)',borderRadius:12}}>
          <div style={{fontWeight:700,fontSize:13,marginBottom:4}}>Download Links</div>
          <div style={{color:'rgba(255,255,255,.4)',fontSize:11,marginBottom:12,lineHeight:1.5}}>
            Tap <b style={{color:'#fff'}}>Copy & Open</b> to copy the link and open it. Then paste in <b style={{color:'#fff'}}>Vidmate</b> or <b style={{color:'#fff'}}>IDM</b> to download.
          </div>
          <div style={{display:'flex',flexDirection:'column',gap:8}}>
            {getExternalDlLinks('movie', item.id).map((src,i)=>(
              <div key={i} style={{background:'rgba(255,255,255,.05)',border:'1px solid rgba(255,255,255,.09)',borderRadius:9,padding:'10px 12px'}}>
                <div style={{fontWeight:700,fontSize:13,color:'#fff',marginBottom:3}}>{src.name}</div>
                <div style={{color:'rgba(255,255,255,.4)',fontSize:11,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',marginBottom:8}}>{src.url}</div>
                <div style={{display:'flex',gap:7}}>
                  <button className="btn" onClick={()=>{ navigator.clipboard?.writeText(src.url).catch(()=>{}); }}
                    style={{flex:1,background:'rgba(255,255,255,.08)',border:'1px solid rgba(255,255,255,.12)',color:'#fff',borderRadius:7,padding:'7px 0',fontSize:12,fontWeight:700,display:'flex',alignItems:'center',justifyContent:'center',gap:5}}>
                    <Ic.Copy/> Copy Link
                  </button>
                  <button className="btn" onClick={()=>copyAndOpen(src.url)}
                    style={{flex:1,background:'#e50914',border:'none',color:'#fff',borderRadius:7,padding:'7px 0',fontSize:12,fontWeight:700,display:'flex',alignItems:'center',justifyContent:'center',gap:5}}>
                    <Ic.Share/> Open
                  </button>
                </div>
          ...
          <button className="btn" onClick={()=>setMovDlStep(null)} style={{marginTop:10,background:'none',border:'none',color:'rgba(255,255,255,.4)',fontSize:12}}>Close</button>
        </div>
      )}
                  ...
                  <div key={ep.episode_number} style={{marginBottom:16,background:'rgba(255,255,255,.03)',border:'1px solid rgba(255,255,255,.07)',borderRadius:12,overflow:'hidden'}}>
                    ...
                    {dlStep === 'links' && (
                      <div style={{padding:'10px 12px 12px',borderTop:'1px solid rgba(255,255,255,.07)',background:'rgba(0,0,0,.3)'}}>
                        <div style={{color:'rgba(255,255,255,.6)',fontSize:12,fontWeight:700,marginBottom:8}}>
                          Download S{season}E{ep.episode_number} — Copy link → Paste in Vidmate or IDM
                        </div>
                        <div style={{display:'flex',flexDirection:'column',gap:7}}>
                          {getExternalDlLinks('tv', item.id, season, ep.episode_number).map((src,li)=>(
                            <div key={li} style={{background:'rgba(255,255,255,.04)',border:'1px solid rgba(255,255,255,.08)',borderRadius:8,padding:'8px 10px'}}>
                              <div style={{fontWeight:700,fontSize:12,color:'#fff',marginBottom:4}}>{src.name}</div>
                              <div style={{display:'flex',gap:6}}>
                                <button className="btn" onClick={()=>navigator.clipboard?.writeText(src.url).catch(()=>{})}
                                  style={{flex:1,background:'rgba(255,255,255,.08)',border:'1px solid rgba(255,255,255,.1)',color:'#fff',borderRadius:6,padding:'6px 0',fontSize:11,fontWeight:700,display:'flex',alignItems:'center',justifyContent:'center',gap:4}}>
                                  <Ic.Copy/> Copy
                                </button>
                                <button className="btn" onClick={()=>window.open(src.url,'_blank')}
                                  style={{flex:1,background:'#e50914',border:'none',color:'#fff',borderRadius:6,padding:'6px 0',fontSize:11,fontWeight:700,display:'flex',alignItems:'center',justifyContent:'center',gap:4}}>
                                  <Ic.Share/> Open
                                </button>
                              </div>
                            </div>
                        ...
                        <button className="btn" onClick={()=>cancelEpDl(ep.episode_number)} style={{marginTop:8,background:'none',border:'none',color:'rgba(255,255,255,.35)',fontSize:11}}>Close</button>
                      </div>
                    )}
                  </div>
                );
